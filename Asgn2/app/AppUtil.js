...
export function buildCCPOrg1() {
	...
	const ccpPath = resolve('../../', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
...
export function buildCCPOrg2() {
	// load the common connection configuration file
	const ccpPath = resolve('../../', 'test-network', 'organizations', 'peerOrganizations', 'org2.example.com', 'connection-org2.json');
	const fileExists = existsSync(ccpPath);
	if (!fileExists) {
		throw new Error(`no such file or directory: ${ccpPath}`);
	}
